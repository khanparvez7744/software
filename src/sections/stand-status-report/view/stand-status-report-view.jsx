...
import StandStatusReportTableRow from '../stand-status-report-table-row';
import StandStatusReportTableHead from '../stand-status-report-table-head';

// ----------------------------------------------------------------------

export default function StandStatusReportPage({ open, onClose }) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const response = await fetch(`${SERVER_IP}/api/reports/get-device-reports/stand-status`, {
      ...
      setStandStatus(data); // Set the retrieved data to the state
    } catch (error) {
  ...
  const prepareCSVData = () => {
    const headers = [
      ...
      { label: 'Stand Status', key: 'standStatus' },
            ...
            >
              ...
              <Grid item xs={12} sm={4}>
                <Stack direction="row" justifyContent="right" spacing={2}>
                  ...
                  <CSVLink
                    filename={`flight_status_report_${new Date().toISOString()}.csv`}
                    {...prepareCSVData()}
              ...
              <Table>
                <StandStatusReportTableHead
                  ...
                  headLabel={[
                    ...
                    { id: 'standStatus', label: 'Stand Status' },
                ...
                <TableBody>
                  ...
                  <TableEmptyRows
                    height={77}
                    emptyRows={emptyRows(page, rowsPerPage, standStatus.length)}
                  />
...
StandStatusReportPage.propTypes = {
  open: PropTypes.bool,
